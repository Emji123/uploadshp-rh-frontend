...
const ShapefileUPSAForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (fileError) {
        console.error('Upload error (UPSA):', fileError);
      ...
      const response = await fetch(`${BACKEND_URL}/validate-shapefile`, {
        ...
        body: JSON.stringify({ zip_path: filePath, bucket: 'rhlupsa' })
      });
      ...
      console.error('Error umum (UPSA):', err);
  ...
  return (
    <div className="form-container">
      <h2>Upload Shapefile RHL UPSA</h2>
      ...
      <form onSubmit={handleSubmit}>
        <div className="input-group">
          ...
          <button type="submit" disabled={isUploading} className="btn-pink">
            Unggah
          </button>
        </div>
      </form>
